<template>
  ...
  <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="form-container">
    <el-form-item label="上传封面 (第一张会做为首页展示)" prop="cover">
      <el-upload ref="upload" class="avatar-uploader" list-type="picture-card" :action="coverUploadUrl" :multiple="true"
        :before-upload="beforeCoverUpload" :on-success="handleCoverUploadSuccess" :on-remove="handleCoverRemove"
        :on-error="handleCoverUploadError" :headers="headers">
    ...
    <el-form-item label="曾获荣誉奖项" prop="honorItems">
      <div v-for="(item, index) in form.honorItems" :key="item.id" class="item">
        <el-form-item :prop="`honorItems.${index}.url`" :rules="{ required: true, message: '请上传内容图片', trigger: 'blur' }">
          <div class="item-uploader">
...
export default {
  ...
  data() {
    return {
      ...
      // 存储页面跳转时是否已经提交数据，如果没有提交，将原来上传的所有图片删除
      submitSuccess: false,
  ...
  methods: {
    ...
    handleCoverRemove(file) {
      ...
      that.$ajax.post('/cos/deleteCosFile', {
        ...
        // 移除表单里的cover图片
        that.form.cover = that.form.cover.filter(item => {
    ...
    deleteAllImage() {
      ...
      this.form.cover.forEach(item => {
        if (item.url != null) {
          pathes.push(item.url.split("\.com")[1])
        }
      })
      this.form.teacherItems.forEach(item => {
        if (item.url != null) {
          pathes.push(item.url.split("\.com")[1])
        }
      })
      this.form.honorItems.forEach(item => {
        if (item.url != null) {
          pathes.push(item.url.split("\.com")[1])
        }
      })
  ...
  beforeDestroy() {
    // 如果页面销毁时，上传的数据仍未提交，则删除所有已上传的数据
    if (!this.submitSuccess) {
      // 删除所有已上传图片, 数据是本地存储的，页面销毁时一并就销毁了，不用删除
      this.deleteAllImage();
    }
  }
}
