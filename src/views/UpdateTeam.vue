<template>
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="form-container">
        <el-form-item label="上传封面 (第一张会做为首页展示)" prop="cover">
            <el-upload ref="upload" class="avatar-uploader" list-type="picture-card" :show-file-list="true"
                :file-list="form.cover" :action="coverUploadUrl" :multiple="true" :before-upload="beforeCoverUpload"
            ...
            <div v-for="(item, index) in form.teacherItems" :key="item.id" class="item">
                <el-form-item :prop="`teacherItems.${index}.url`"
                    ...
                    <div class="item-uploader">
                        <!-- :file-list="item.url == null || item.url == '' ? [] : [item]" 有图片时才显示 -->
                        <el-upload ref="upload" class="item-upload" :action="teacherImageUploadUrl" :headers="headers"
                            ...
                            :file-list="item.url == null || item.url == '' ? [] : [item]" :limit="1"
                            :before-upload="beforeTeacherUpload"
        ...
        <el-form-item label="曾获荣誉奖项" prop="honorItems">
            <div v-for="(item, index) in form.honorItems" :key="item.id" class="item">
                <el-form-item :prop="`honorItems.${index}.url`"
                    ...
                    <div class="item-uploader">
                        <!-- :file-list="item.url == null || item.url == '' ? [] : [item]" 有图片时才显示 -->
                        <el-upload ref="upload" class="item-upload" list-type="picture-card" :show-file-list="true"
                            :file-list="item.url == null || item.url == '' ? [] : [item]" :action="honorImageUploadUrl"
        ...
        <el-form-item class="button_center">
            ...
            <!-- 修改时不提供重置按钮，其实是有个bug不想改了 重置的时候，需要清空所有表单，我现在清空不了，不想清空了 -->
...
export default {
    ...
    methods: {
        ...
        handleCoverRemove(file, fileList) {
            ...
            that.$ajax.post('/cos/deleteCosFile', {
                ...
                // 移除表单里的cover图片
                that.form.cover = that.form.cover.filter(item => { return item.url != url })
                that.$message({
                    duration: 1000,
                    showClose: true,
                    message: '删除成功',
        ...
        handleHonorItemOnChange(file, fileList, index) {
            // 这里不知道什么情况 写this.hideHonorImgeAdd.splice(index, 1, fileList.length >= this.limitCount);这段代码就会报错，其他方式实现就可以
            // this.hideHonorImgeAdd.splice(index, 1, fileList.length >= this.limitCount);
            // 但是这种理论上vue是不会接管的，不知道这里为什么
            this.hideHonorImgeAdd[index] = fileList.length >= this.limitCount;
        },
        // 荣誉图片移除
        ...
        handleTeacherItemOnChange(file, fileList, index) {
            // 这里也是一样，使用splice函数就会报错
            // this.hideTeacherImgeAdd.splice(index, 1, fileList.length >= this.limitCount);
            // vue文档里，本身这部分是不渲染的，不知道为什么这里有用
            this.hideTeacherImgeAdd[index] = fileList.length >= this.limitCount;
        },
        ...
        submitForm() {
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    that.$ajax.post('/team/updateTeam', data)
                        .then(function (response) {
                            that.$message({
                                duration: 1000,
                                showClose: true,
        ...
        resetForm() {
            ...
            // 页面无抖动刷新
            // that.reload()
            this.form.honorItems = []
            this.form.teacherItems = []
            this.hideTeacherImgeAdd = []
            this.hideHonorImgeAdd = []
    ...
    mounted() {
        let that = this
        that.$ajax.get(`/team/teamUpdateInfoByTeamId?teamId=${that.teamId}`)
            .then(function (response) {
                console.log(response)
                that.form.title = response.teamInfo.teamName
                that.form.description = response.teamInfo.teamIntroduction
                that.form.cover = response.teamInfo.cover
                that.form.college = response.teamInfo.college
                that.form.teamType = response.teamInfo.teamType
                that.form.teamCreateTime = response.teamInfo.teamCreateTime
                that.form.teacherItems = response.teamInfo.teachers
                that.form.honorItems = response.teamInfo.achievements
                // 对应上面隐藏+号的数据
                that.form.honorItems.forEach(item => {
                    that.hideHonorImgeAdd.push(true)
                })
                that.form.teacherItems.forEach(item => {
                    that.hideTeacherImgeAdd.push(true)
                })
